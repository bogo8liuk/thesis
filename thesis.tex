...
\[ x : \sigma \]
afferma che il simbolo $ x $ ha tipo $ \tau $. %TODO
...
(\texttt{True}) o semplicemente l'algoritmo di unificazione (\texttt{False}). Un test di specializzazione è utile per
capire se l'unificazione può essere fatta in un solo "verso", ovvero se, dati due mono-tipi $ \tau $ e $ \tau' $ e data
la sostituzione di ritorno $ S = \{ \alpha_i \mapsto \tau_i \} $ conseguente alla valutazione dell'unificazione di
$ \tau $ e $ \tau' $, vale:
\[ \forall \alpha_i \in S. \; \alpha_i \in \tau' \]
dove con $ \alpha_i \in S $ si intendono solamente le variabili di tipo nella parte sinistra di una sostituzione, ovvero
quelle che devono essere sostituite. Questo rende l'unificazione più stringente
