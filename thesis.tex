...
Martin Odersky, Martin Wehr presentano un'estensione di HM che supporta anche i constraints. I giudizi di tipo vengono
estesi con un'ipotesi aggiuntiva sui constraints e avranno la forma:
\[ C, \Gamma \vdash_W e : \tau \]
Perciò l'ambiente di tipizzazione viene esteso con un altro "attore", ovvero un contesto sui constraints. L'articolo
presenta inizialmente il problema di come ridefinire le regole istanziazione (\textbf{Inst}) e generalizzazione
(\textbf{Gen}). Sull'istanziazione di uno schema di tipo si arriva subito alla seguente conclusione:
\[ \infer[\textbf{Inst}]{C, \Gamma \vdash_W e : \{ \overline{\tau} / \overline{\alpha} \}\tau}{C, \Gamma \vdash_W e : \forall \overline{\alpha}. D \Rightarrow \tau & C \vdash^e \{ \overline{\tau} / \overline{\alpha} \} D} \]
Questa regola sostiene che, quando si istanzia uno schema di tipo $ \forall \overline{\alpha}. D \Rightarrow \tau $, le
uniche istanze valide sono quelle della forma $ \{ \overline{\tau} / \overline{\alpha} \} $ che \textit{soddisfano}
il constraint $ D $ nello schema di tipo. In langgg, un constraint $ D $ è \textit{soddisfatto} se, nel programma
(formalemente in $ C $), esiste un'istanza $ D' $ (in questo caso si intendono le istanze langgg, non l'istanziazione
di un tipo) tale che:
\[ satisfy(D, D') \]
dove $ satisfy $ è l'algoritmo di soddisfabilità dei constraints, definito in \texttt{Compiler.Ast.Typed}. Esso si
basa sull'unificazione \textit{strict}:
\begin{lstlisting}
satisfy(C args, C' args'):
    C == C' and
    areEqAfterSpecTest(args, args')
\end{lstlisting}
dove \texttt{areEqAfterSpecTest} è l'algoritmo che effettua il test di specializzazione tra i vari tipi di \texttt{args}
e \texttt{args'}:
\begin{lstlisting}
areEqAfterSpecTest(args, args'):
...
Si ricordi che il test di unificazione può fallire. Si noti
come, a causa dell'utilizzo dell'unificazione \textit{strict}, il test soddisfabilità tra constraints, non sia
...
regola di generalizzazione. Nell'articolo [2], vengono citati altri sistemi di tipi con constraints i quali, a
differenza della regola di istanziazione, non convergono verso un'unica regola di inferenza. Le disparità si trovano
nella'applicazione di soddisfabilità al momento della generalizzazione. In [2], viene proposta la seguente regola:
\[ \infer[\textbf{Gen}]{C \wedge \exists \overline{\alpha}. D, \Gamma \vdash_W e : \forall \overline{\alpha}. D \Rightarrow \tau}{C \wedge D, \Gamma \vdash_W e : \tau & \overline{\alpha} \notin free(\Gamma) \cup free(C)} \]
Si noti come vi è un nuovo quantificatore: $ \exists \overline{\alpha}. D $. Questa notazione è utile per quantificare
le variabili di tipo all'interno di un constraint. In langgg, questo permette di "rimuovere" un constraint dall'ambiente
di tipizzazione.
