...
etc.. Questo tipo di check è molto importante in quanto fasi successive del compilatore basano le loro computazioni
...
\begin{enumerate}
    \item $\forall$ occorrenza di nome di tipo =: \textit{name}:
        \[ \#args(name) \leq \#args(def(name)) \]
    \item $\forall$ occorrenza di nome di alias di tipo =: \textit{name}:
        \[ \#args(name) = \#args(def(name)) \]
    \item $\forall$ occorrenza di nome di proprietà =: \textit{name}:
        \[ \#args(name) = \#args(def(name)) \]
\end{enumerate}
dove \textit{def} è la funzione che ritorna la definizione di un token nell'AST, \textit{args} è la funzione che calcola
gli argomenti di un token e il simbolo \textit{\#} è la funzione che calcola la cardinalità di un insieme. La condizione
(1) è meno stringente di (2) e di (3), in quanto l'utente può "manipolare" non solo tipi, ma anche funzioni di tipi,
