...
\[ x : \sigma \]
afferma che il simbolo $ x $ ha schema di tipo $ \sigma $, mentre:
\[ x : \tau \]
afferma che il simbolo $ x $ ha mono-tipo $ \tau $. Ora verranno elencate le regole di inferenza:
...
\[ \infer[\textbf{Spec}]{\forall \alpha_1 ... \forall \alpha_n. \; \tau \sqsubseteq \forall \beta_1 ... \beta_m. \; \tau'}{\tau' = \{ \alpha_i / \tau_i \}\tau & \beta_i \notin free(\forall \alpha_1 ... \forall \alpha_n . \; \tau)} \]
La prossima regola rappresenta l'algoritmo di instanziazione di schema di tipo a mono-tipo:
...
Questa prima regola è stata già introdotta, seppur in maniera informale, richiede l'introduzione di un'ulteriore regola,
ovvero l'instanziazione di uno schema di tipo ($ inst $).
dove $ \sqsubseteq $ è una relazione d'ordine parziale
